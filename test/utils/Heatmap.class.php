...
class Heatmap {
	...
	/** @var string $host H么te (serveur) MySQL / MySQL host (server) */
	var $host = 'localhost';
	...
	var $password = '';
	/** @var string $database Nom de la base de donn茅es MySQL / MySQL database name */
	var $database = '';
	/** @var integer $limit Limite du nombre de r茅sultats renvoy茅s par la requ锚te �� chaque appel / Maximum number of results returned by each request call */
	var $limit = 1000;
	/** @var resource $link Lien (interne) MySQL / MySQL (internal) link */
	var $link = false;
	/** @var string $query Requ锚te renvoyant les coordonn茅es des clics / Clicks coordinates query */
	var $query = 'SELECT COORDS_X, COORDS_Y FROM CLICKS WHERE COORDS_Y BETWEEN %d AND %d';
	/** @var string $maxQuery Requ锚te renvoyant la coordonn茅es Y maximale / Max Y coordinate query */
	 ...
	 *
	 * @param integer $image
	 *        	Number of the image (to be used with $this->height)
	...
	function drawPixels($image) {
		...
		do {
			 ...
			 * Select with limit
			 */
			$query = sprintf ( $this->query, $image * $this->height, ($image + 1) * $this->height - 1 ) . ' LIMIT ' . $limit . ',' . $this->limit;
			/*
			 * 打印出来的内容为： SELECT CLICK_X, CLICK_Y FROM CLICKS WHERE CLICK_Y BETWEEN 0 AND 99 LIMIT 0,1000 SELECT CLICK_X, CLICK_Y FROM CLICKS WHERE CLICK_Y BETWEEN 100 AND 199 LIMIT 0,1000
			 */
